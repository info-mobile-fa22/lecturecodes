  ...
  return (
    <div className="App">
      <header className="App-header">
        <ColorContext.Provider value={{linkColor:"#335c67", titleColor:"#9e2a2b"}}>
...
function TimePanel(props){
  ...
  return(
      ...
      <span class="status"> Total {number} months of reviews</span>

        {months.map(month =><a class="status" href="#" style={{color:colorContext.linkColor}}>{month}</a>)}
...
function ControlButton(props){
  const colorContext = useContext(ColorContext)
  return(<a  href="#" class="status" style={{color:colorContext.linkColor}} id={props.id} onClick = {props.clickFunction}>{props.label} </a>)
}
...
function Title(props){
  const colorContext = useContext(ColorContext)
  return(
    <div class="caption-display" style={{color:colorContext.titleColor}}>
